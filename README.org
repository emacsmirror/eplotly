...
* Piechart
  ...
  To create a simple pie chart, pass a data series with the following parameters:

  - values : values for each slice of the piechart
  - labels : labels for each slice of the piechart
  - type: set to "pie"
...
* Histograms
 ...
 For vertical histograms, the data-series list must contain
 a nested list with car equal 'x and 'type equal to "histogram":
...
#+end_src
  ...
  For horizontal histograms, use 'y instead of 'x for
